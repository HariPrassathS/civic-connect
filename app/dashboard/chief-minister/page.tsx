...
import { getChiefMinisterMetrics } from "./queries";
...
export default async function ChiefMinisterDashboard() {
  const metrics = await getChiefMinisterMetrics();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">CM Dashboard</h2>
        <p className="text-muted-foreground">
          Top-level state analytics, SLA compliance across all districts, and strategic AI insights.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-7">
        ...
        <Card className="md:col-span-3">
          <CardHeader>
            ...
            <CardDescription>
              Issues escalated to the Chief Minister level
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-4">
                {metrics.escalatedComplaints.map((c: any) => (
                  <div key={c.id} className="flex flex-col justify-between gap-4 rounded-lg border p-4 sm:flex-row sm:items-center">
                    ...
                    <Link href={`/dashboard/chief-minister/complaints/${c.id}`}>
