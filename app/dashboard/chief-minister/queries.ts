...
export async function getChiefMinisterMetrics() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return null;

  // Chief Minister sees everything across the entire state
  ...
  if (error || !complaints) {
    console.error("Error fetching complaints for Chief Minister:", error);
    return null;
  }
