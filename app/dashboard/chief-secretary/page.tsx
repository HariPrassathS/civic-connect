...
import { getChiefSecretaryMetrics } from "./queries";
...
export default async function ChiefSecretaryDashboard() {
  const metrics = await getChiefSecretaryMetrics();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">State Overview</h2>
        <p className="text-muted-foreground">
          State-wide analytics, SLA compliance, and AI insights across all districts.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-7">
        ...
        <Card className="md:col-span-3">
          <CardHeader>
            ...
            <CardDescription>
              Issues escalated to the Chief Secretary level
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-4">
                {metrics.escalatedComplaints.map((c: any) => (
                  <div key={c.id} className="flex flex-col justify-between gap-4 rounded-lg border p-4 sm:flex-row sm:items-center">
                    ...
                    <Link href={`/dashboard/chief-secretary/complaints/${c.id}`}>
