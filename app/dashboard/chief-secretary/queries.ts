...
export async function getChiefSecretaryMetrics() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return null;

  // Chief Secretary sees everything across the state
  ...
  if (error || !complaints) {
    console.error("Error fetching complaints for Chief Secretary:", error);
    return null;
  }
