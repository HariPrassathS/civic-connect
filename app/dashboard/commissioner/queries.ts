...
export async function getCommissionerMetrics() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return null;

  // Commissioner sees everything across all departments in the city
  ...
  if (error || !complaints) {
    console.error("Error fetching complaints for Commissioner:", error);
    return null;
  }
