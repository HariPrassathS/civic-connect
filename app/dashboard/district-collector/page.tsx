...
import { getDistrictCollectorMetrics } from "./queries";
...
export default async function DistrictCollectorDashboard() {
  const metrics = await getDistrictCollectorMetrics();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">District Overview</h2>
        <p className="text-muted-foreground">
          District-wide analytics, SLA compliance, and AI insights across all departments.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-7">
        ...
        <Card className="md:col-span-3">
          <CardHeader>
            ...
            <CardDescription>
              Issues escalated to the Collector level
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="space-y-4">
                {metrics.escalatedComplaints.map((c: any) => (
                  <div key={c.id} className="flex flex-col justify-between gap-4 rounded-lg border p-4 sm:flex-row sm:items-center">
                    ...
                    <Link href={`/dashboard/district-collector/complaints/${c.id}`}>
